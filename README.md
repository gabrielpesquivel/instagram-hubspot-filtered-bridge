...
- **Gangsheet Generator** (`#/gangsheet`) — turns Shopify order CSVs into print-ready gangsheet PDF/AI files. Runs the desktop tool's exact Python code in the browser via Pyodide (WebAssembly); no server-side processing. Source of truth is `../BootinkGangsheetGenerator` — `scripts/build-gangsheet-bundle.sh` packages its `app/` code and `assets/` into `dashboard/public/gangsheet/bundle.zip` (run automatically by `npm run deploy`; rerun after changing the generator). The desktop tool runs `rsvg-convert` to render gradient/clipPath SVGs as vector PDF. The browser has no such binary, so `build-gangsheet-bundle.sh` runs `rsvg-convert` at build time and ships a `<file>.svg.pdf` vector sidecar for each of those SVGs; `gangsheet-web/web_runner.py` embeds them exactly as the desktop does, making the web output byte-identical to the exe. (The canvas rasterizer in `dashboard/src/Gangsheet.tsx` remains only as a fallback if a sidecar is missing.) **Building the bundle therefore requires `rsvg-convert`** (macOS: `brew install librsvg`).
  - **Sheet sizes**: 580mm wide. First sheet is 550mm tall (quick-start, prints sooner), later sheets 850mm, and the last sheet is cropped to its content + 50mm margins (see the generator repo's README / `layout.py`).
